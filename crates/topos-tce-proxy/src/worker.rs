...
impl TceProxyWorker {
    /// Construct a new [`TceProxyWorker`] with a 2048 items deep channel to send commands to and receive events from a TCE node on the given subnet.
    /// The worker holds a [`crate::client::TceClient`]
    pub async fn new(config: TceProxyConfig) -> Result<(Self, Option<(Certificate, u64)>), Error> {
        let (command_sender, mut command_rcv) = mpsc::channel::<TceProxyCommand>(2048);
        let (evt_sender, evt_rcv) = mpsc::channel::<TceProxyEvent>(2048);
